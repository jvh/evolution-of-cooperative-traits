...
def add_to_dataframe(data, gen, ssr, slr, csr, clr):
    data.loc[-1] = ({'generation': gen, 'ss': ssr, 'sl': slr, 'cs': csr, 'cl': clr})
    data.index = data.index + 1  # shifting index


def rescale_population(population, gen, data, print_population=True):
    """
    ...
    # Add to dataframe
    add_to_dataframe(data, gen, ssr, slr, csr, clr)
